...
if [[ ${GPU} == rocm* ]]; then
	...
	sed -i '179,183d' 3rdparty/flashinfer/include/flashinfer/utils.cuh
	sed -i '184,188d' 3rdparty/flashinfer/include/flashinfer/utils.cuh
	sed -i '203,207d' 3rdparty/flashinfer/include/flashinfer/utils.cuh
else
	echo set\(USE_LLVM \"llvm-config --ignore-libllvm --link-static\"\) >>config.cmake
fi
