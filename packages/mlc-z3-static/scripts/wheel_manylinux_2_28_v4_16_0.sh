...
if [[ "$(uname -s)" != "Linux" ]]; then
  echo "wheel_manylinux_2_28_v4_16_0.sh must run on Linux with Docker available" >&2
  exit 1
fi
...
case "${MLC_Z3_STATIC_ARCH}" in
  x86_64|aarch64) ;;
  *)
    echo "Unsupported manylinux architecture: ${MLC_Z3_STATIC_ARCH}" >&2
    exit 1
    ;;
esac
...
: "${MLC_Z3_STATIC_PLAT_NAME:=manylinux_2_28_${MLC_Z3_STATIC_ARCH}}"

export CIBW_BUILD="${CIBW_BUILD:-cp311-manylinux_${MLC_Z3_STATIC_ARCH}}"
export CIBW_ARCHS_LINUX="${CIBW_ARCHS_LINUX:-${MLC_Z3_STATIC_ARCH}}"
